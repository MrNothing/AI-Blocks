...
def Run(self, graph):
	...
	if self.type=="GRU":
		...
		val = tf.transpose(val, [1, 0, 2])
		last = tf.gather(val, int(val.get_shape()[0]) - 1)
		weight = tf.Variable(tf.truncated_normal([self.hidden_units[len(self.hidden_units)-1], self.n_classes]), name=self.name+"_W")
		bias = tf.Variable(tf.constant(0.1, shape=[self.n_classes]), name=self.name+"_b")
